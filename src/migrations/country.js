...
try{
	...
	await Truncate(destDb,"smartirb.country")
				

		console.log("Migrating Table country...")
		let success = 0 
		let errors = 0 
		try {
			const [rows] = await sourceDb.execute('SELECT * FROM country');
			for (let row of rows) {
				...
				await destDb.none('INSERT INTO smartirb.country (id, name, abbreviation) VALUES ($1, $2, $3)', [row.id, row.name, row.abbreviation]).then(
					(res)=>{
						success++
					}
				)
			...
			console.log('Migration of country table completed!');
			console.log('Success Count::', success);
			console.log('Error Count::', errors);

		} catch (error) {
			console.error('Migration of country failed:', error);
		} finally {
			await sourceDb.end();
